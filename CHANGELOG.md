...
- **Web 控制台日志丢失问题**（`apps/web/server.py`）
  - 将 `asyncio.Queue` 替换为线程安全的 `queue.Queue`
  - 使用 `run_in_executor` 消费队列，避免阻塞事件循环
  - 解决同步代码执行期间日志无法正确推送到前端的问题

- **富文本模板分隔符间距丢失**（`templates/rich_text_template.html`）
  - 在分隔符中添加透明占位元素（`&nbsp;`）
  - 避免微信 API 移除空元素导致文章间距消失

### 其他

- **桌面客户端版本号同步**（`apps/desktop/main_window.py`）
  - 修正 v2.2.1 遗漏的版本号更新
...
小版本更新：修复 Web 控制台环境变量加载问题，优化 HTML 标准化与评分规则。

### Bug 修复

- **Web 控制台环境变量加载**（`web_app.py`）
  - 启动时加载 `.env` 文件，修复环境变量无法正确读取的问题

### 优化改进

- **HTML 标准化逻辑**（`utils/wechat/html_normalizer.py`）
  - 简化样式重置逻辑，统一使用 `margin: 0; padding: 0; text-indent: 0;` 重置
  - 增强文本节点清理，去除源码格式化产生的缩进，避免发布时出现多余空白

- **文章评分规则**（`workflows/daily_generate.py`）
  - 调整基础分数上限从 3 分改为 2 分
  - 移除"文章是 AI 领域"的基础得分点，使评分更加严格
