...
# 导入工作流模块（从 src 中导入）
from wechat_ai_daily.workflows import DailyGenerator, ArticleCollector, DailyPublisher
...
class WorkflowWorker(QThread):
    ...
    async def _run_collect(self) -> None:
        ...
        try:
            # 创建采集器
            collector = ArticleCollector(config=self.config_path)
    ...
    async def _run_publish(self) -> None:
        ...
        try:
            ...
            self.finished_signal.emit(True, "草稿发布完成", f"draft:{draft_media_id}")
    ...
    async def _run_full(self) -> None:
        ...
        # 阶段1：采集文章
        self.progress.emit(0, "阶段 1/3: 采集公众号文章", "初始化采集器...")
        
        try:
            collector = ArticleCollector(config=self.config_path)
            
            self.progress.emit(5, "阶段 1/3: 采集公众号文章", "采集器已初始化，开始采集...")
            
            markdown_file = await collector.run(target_date=self.target_date)
            
            if self._is_cancelled:
                self.finished_signal.emit(False, "用户取消了操作", "")
                return
            
            self.progress.emit(33, "阶段 1/3: 采集完成", f"已采集到文章链接: {markdown_file}")
        ...
        try:
            ...
            self.finished_signal.emit(True, "完整工作流执行完成", f"draft:{draft_media_id}")
